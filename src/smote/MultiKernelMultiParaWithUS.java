...
public class MultiKernelMultiParaWithUS {
	public static void main(String[] args) throws Exception {
		// TODO Auto-generated method stub
		String[] dataSets = { "ecoli1", "ecoli2", "ecoli3", "ecoli4", "ecoli0v1", "ecoli0137v26", "yeast1", "yeast3",
				"yeast4", "yeast5", "yeast6", "glass0", "glass1", "glass2", "glass4", "glass5", "glass6", "glass016v2",
				"glass016v5", "glass0123vs456", "newthyroid1", "newthyroid2", "pima" };

	//	 String[] dataSets = {"newthyroid2"};
...
//			System.out.println(dataSets[dataset]);
			...
			for (int cnt = 0; cnt < 20; ++cnt) {
				...
				for (int fold = 0; fold < 5; ++fold) {
					...
					for (int k = 0; k < thetapara.length; ++k) {
						for (int i = 0; i < 6; ++i) {
							for (int j = 0; j < SETTING.K; ++j) {
								...
								J45.buildClassifier(allParaKernel.get(k).get(i).get(j));
								// System.out.println("大小："+minoritySamples.size()+" "+majoritySamples.size() +"
								// "+ allKernel.get(i).get(j).size());
					 ...
					 */
					//欠采样K个集合
					List<Instances> USSamples = new ArrayList<Instances>();
					for(int i = 0; i < SETTING.K; ++i) {
						Instances temp = new Instances(data);
						temp.clear();
						for(Instance inst:minoritySamples) {
						...
						int[] flagMaj = new int[majoritySamples.size()];
						for(int j = 0; j < minoritySamples.size(); ++j) {
						
							Random random = new Random();
							int index = random.nextInt(majoritySamples.size());
							if(flagMaj[index] == 1) {continue;}
							temp.add(majoritySamples.get(index));
							flagMaj[index] = 1;
						}
						USSamples.add(temp);
					}
					...
					Classifier[] classifier = new Classifier[SETTING.K*2];
					...
					for (int i = 0; i < SETTING.K; ++i) {
						// classifier[i] = new AdaBoostM1();
						classifier[SETTING.K+i] = new J48();
						// classifier[i].setOptions(options);
						classifier[SETTING.K+i].buildClassifier(
								USSamples.get(i));
					}
			...
			System.out.println(dataSets[dataset]+": "+finalscore/20);
		}
