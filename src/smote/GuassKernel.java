...
public class GuassKernel {
	...
	public static double kernelGenerateDistance1(Instance inst, Instance i, Instance j, double gap) {
		...
		distance =(gap-1)*(gap-1)*gauss(i,i)+(2*gap-2*gap*gap)*gauss(i,j)+gap*gap*gauss(j,j)+gauss(inst,inst)+
				2*(gap-1)*gauss(inst,i)-2*gap*gauss(inst,j);
		return distance;
	}
		...
		for (int th = 0; th < thetapara.length; ++th) {
			...
			for (int cnt = 0; cnt < 1; ++cnt) {
				...
				for (int fold = 0; fold < 5; ++fold) {
					...
					for (int i = 0; i < SETTING.K; ++i) {
						 classifier[i] = new weka.classifiers.bayes.NaiveBayes();
