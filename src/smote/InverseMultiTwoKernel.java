...
//逆多元二次核
public class InverseMultiTwoKernel {
			...
			while (IR != 0) {
				...
				if (flag == 1) {
					...
					for(int u = 0; u < value.length; ++u) {
						System.out.println(String.format("%.3f",value[i]));
					}
					}
					IR--;
	...
	public static List<Double> generateDistance(List<Double> distance, int flag){
		for (int i = 0; i < distance.size(); ++i) {
			...
			*/
			double temp = (1.0/(1-0.5*distance.get(i)))*(1.0/(1-0.5*distance.get(i)))-SETTING.c*SETTING.c;
			distance.set(i, temp);
	...
	public static double kernelGenerateDistance(Instance inst, Instance samplei, Instance samplej, double gap) {
		double distance;
		distance = (1 + 2 * gap) * inversekernel(inst, inst) - 2 * inversekernel(inst, samplei) - 2 * gap * inversekernel(inst, samplej)
				+ (gap - 1) * (gap - 1) * inversekernel(samplei, samplei) + 2 * gap * (1 - gap) * inversekernel(samplei, samplej)
				+ gap * gap *inversekernel(samplej, samplej);
		return distance;
	}
	...
	public static double kernelGenerateDistance1(Instance inst, Instance i, Instance j, double gap) {
		...
		distance =(gap-1)*(gap-1)*inversekernel(i,i)+(2*gap-2*gap*gap)*inversekernel(i,j)+gap*gap*inversekernel(j,j)+gauss(inst,inst)+
				2*(gap-1)*inversekernel(inst,i)-2*gap*inversekernel(inst,j);
		return distance;
	}
	...
	public static List<Integer> kNearestNeihbors(Instances inputData, Instance inst) {
		...
		for (int i = 0; i < inputData.size(); ++i) {
			distances.add(inversekernelDistance(inst, inputData.get(i)));
		}
	...
	//线性和算距离
	public static double inversekernelDistance(Instance a, Instance b) {
		double result = 0.0f;
		result = inversekernel(a, a) - 2 * inversekernel(a, b) + inversekernel(b, b);
		return result;
	}
	...
	//镜像基核函数
	public static double inversekernel(Instance a, Instance b) {
		...
		temp =1.0/ Math.sqrt(temp+SETTING.c*SETTING.c);
		
	//	System.out.println(temp+" "+SETTING.c+" "+SETTING.c*SETTING.c + "-"+1.0/(SETTING.c*SETTING.c));
		return temp;
	}
	//
