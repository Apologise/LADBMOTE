...
public class LaplaKernel {
	...
	public static List<Double> generateDistance(List<Double> distance, int flag){
		for (int i = 0; i < distance.size(); ++i) {
			...
			double temp = -2 * SETTING.theta * (Math.log(1 - 0.5 * distance.get(i)));
	...
	public static double kernelGenerateDistance(Instance inst, Instance samplei, Instance samplej, double gap) {
		double distance;
		distance = (1 + 2 * gap) * lapkernel(inst, inst) - 2 * lapkernel(inst, samplei) - 2 * gap * lapkernel(inst, samplej)
				+ (gap - 1) * (gap - 1) * lapkernel(samplei, samplei) + 2 * gap * (1 - gap) * lapkernel(samplei, samplej)
				+ gap * gap *lapkernel(samplej, samplej);
		return distance;
	}
	...
	public static double kernelGenerateDistance1(Instance inst, Instance i, Instance j, double gap) {
		...
		distance =(gap-1)*(gap-1)*lapkernel(i,i)+(2*gap-2*gap*gap)*lapkernel(i,j)+gap*gap*lapkernel(j,j)+lapkernel(inst,inst)+
				2*(gap-1)*lapkernel(inst,i)-2*gap*lapkernel(inst,j);
		return distance;
	}
	...
	public static List<Integer> kNearestNeihbors(Instances inputData, Instance inst) {
		...
		for (int i = 0; i < inputData.size(); ++i) {
			distances.add(lapkernelDistance(inst, inputData.get(i)));
		}
	...
	//线性和算距离
	public static double lapkernelDistance(Instance a, Instance b) {
		double result = 0.0f;
		result = lapkernel(a, a) - 2 * lapkernel(a, b) + lapkernel(b, b);
		return result;
	}

	
	//镜像基核函数
	public static double lapkernel(Instance a, Instance b) {

		double temp = 0.0f;
		for (int i = 0; i < a.numAttributes() - 1; ++i) {
			temp += (a.value(i) - b.value(i)) * (a.value(i) - b.value(i));
		}
		temp = Math.exp(-Math.sqrt(temp) / (2* SETTING.theta));
		return temp;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		/*String[] dataSets = { "ecoli1", "ecoli2", "ecoli3", "ecoli4","ecoli0v1","ecoli0137v26", "yeast1", "yeast3", "yeast4", "yeast5", "yeast6",
				...
				"pima" };*/
		String[] dataSets = { "glass1","ecoli0v1","pima","glass0","yeast1",
				"vehicle1",
