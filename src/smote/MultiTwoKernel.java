...
public class MultiTwoKernel {
	...
	public static List<Double> generateDistance(List<Double> distance, int flag){
		for (int i = 0; i < distance.size(); ++i) {
			...
			*/
			double temp = (1-0.5*distance.get(i))*(1-0.5*distance.get(i))-SETTING.c*SETTING.c;
			distance.set(i, temp);
	...
	public static double kernelGenerateDistance(Instance inst, Instance samplei, Instance samplej, double gap) {
		double distance;
		distance = (1 + 2 * gap) * expkernel(inst, inst) - 2 * expkernel(inst, samplei) - 2 * gap * expkernel(inst, samplej)
				+ (gap - 1) * (gap - 1) * expkernel(samplei, samplei) + 2 * gap * (1 - gap) * expkernel(samplei, samplej)
				+ gap * gap *expkernel(samplej, samplej);
		return distance;
	}
	...
	public static double expkernel(Instance a, Instance b) {
		...
		temp = Math.sqrt(temp+SETTING.c*SETTING.c);
		return temp;
	}
